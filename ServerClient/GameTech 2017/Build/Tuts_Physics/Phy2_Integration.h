...
public:
	...
	//Create a projectile
		...
		m2_Sphere = new GameObject("Sphere2");
		m2_Sphere->SetRender(new RenderNode());
		m2_Sphere->Render()->AddChild(sphereRender2);
		m2_Sphere->SetPhysics(new PhysicsNode());
		m2_Sphere->Physics()->SetInverseMass(1.f);
		//Position, vel and acceleration all set in "ResetScene()"
		this->AddGameObject(m2_Sphere);

		//for moving the sphere around
		ScreenPicker::Instance()->RegisterNodeForMouseCallback(
			sphereRender2,
			std::bind(&CommonUtils::DragableObjectCallback, m2_Sphere, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4)
		);
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		m2_Sphere->Physics()->SetOnUpdateCallback([&](const Matrix4& transform)
		{
			m2_Sphere->Render()->SetTransform(transform); //Default callback for any object that has a render and physics nodes
	...
	void ResetScene(float timestep)
	{
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		m2_Sphere->Physics()->SetPosition(Vector3(7.5f, 2.0f, 0.f));
		m2_Sphere->Physics()->SetLinearVelocity(Vector3(0.f, 2.5f, 0.0f));
		m2_Sphere->Physics()->SetForce(Vector3(1.f, -1.f, 0.0f));

		//Cause we can.. we will also spin the ball 1 revolution per second (5 full spins before hitting target)
		// - Rotation is in radians (so 2PI is 360 degrees), richard has provided a DegToRad() function in <nclgl\common.h> if you want as well.
		m2_Sphere->Physics()->SetOrientation(Quaternion());
		m2_Sphere->Physics()->SetAngularVelocity(Vector3(0.f, 0.f, -2.0f * PI));
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void UpdateTrajectory2(const Vector3& pos)
	{
		...
		if (m2_Sphere->Physics()->GetPosition().y < 0.0f)
		{
			PhysicsEngine::Instance()->SetPaused(true);
		}
...
private:
	Mesh*					m_TargetMesh;
	GameObject*				m_Sphere, *m2_Sphere;
	std::vector<Vector3>	m_TrajectoryPoints, m_TrajectoryPoints2;
};