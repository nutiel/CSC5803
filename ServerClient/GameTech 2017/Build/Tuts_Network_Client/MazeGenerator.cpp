...
{
	int a, b;
	a = (int)(st.x + size*st.y);
	b = (int)(en.x + size*en.y);
...
void  MazeGenerator::Generate_Prims()
{
	...
	uint startIdx = (int)(st.x + st.y*size);
	...
	while (walls.size() > 0)
	{
		...
		int a, b;
		a = (int)(edge->_a->_pos.x + edge->_a->_pos.y*size);
		b = (int)(edge->_b->_pos.x + edge->_b->_pos.y*size);
